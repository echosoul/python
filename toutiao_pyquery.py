...
def get_images(save_dir, url):
    ...
    res = urllib.request.urlopen(req).read().decode('utf-8')
    doc = pq(res)
    imgs = doc('.article-content')
    if imgs is not None:
        for it in imgs.items('img'):
            img = it.attr('src')
            save_file = img.split(r'/')[-1] + r'.jpg'
            urllib.request.urlretrieve(
                img, os.path.join(save_dir, save_file))
...
if __name__ == '__main__':
    ...
    while(offset <= 20):
        ...
        article_req = urllib.request.Request(
            query_url, headers=request_headers)
        ...
        for i in range(len(article_titles)):
            if article_titles[i] is not None and article_urls[i] is not '':
                save_dir = _create_dir(
                    os.path.join(root_dir, article_titles[i]))
